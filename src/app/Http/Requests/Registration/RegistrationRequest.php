...
class RegistrationRequest extends FormRequest
{
    ...
    {
        $isUpdate = $this->route()->uri == 'api/update-student';

        return [
            'image' => ['sometimes', 'image','nullable','mimes:jpeg,jpg,png'],
            'student_information' => ['required','array'],
            'student_information.id' => ['sometimes'],
            'student_information.email' => ['required', 'email', $isUpdate ? Rule::unique('students', 'email')->ignore(optional($this->student_information)['id']) : 'unique:students,email'],
